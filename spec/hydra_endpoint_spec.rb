...
RSpec.describe HydraEndpoint do
  ...
  let(:data) { { 'default_work' => { 'title' => 'test' } } }
  ...
  it 'has a new_work_url' do
    expect(subject.new_work_url).to eq work_type_config.dig('hydra_endpoint', 'new_work', 'url')
  end
  ...
  context 'with a mocked csrf_token' do
    ...
    it 'can publish_work' do
      expect(subject).to receive(:post_data).with(subject.new_work_action, JSON.generate(data), headers).and_return(true)
      expect(subject.publish_work(data)).to be_truthy
    end
